...
def connect():
    connection = psycopg2.connect(
        ...
        user="dnew", # dnew, dotvb, dtps, dtms
        password="masquerade", # masquerade, internaluse, abundance, ownedmedia0913
...
def download_img_to_local_folder():
    ...
    for i, j, k in zip(df["datepublish"], df["filename"], df["title"]):
        ...
        img_url = "https://img.digitimes.com/newsimg/{}/{}/{}".format(i.year, i.strftime("%m%d"), j.lower())
        print(img_url)
...
def load_picture():
    folder_path = "C:\\Users\\allen.jiang\\Downloads\\news_imgs\\"
    image_files = [f for f in os.listdir(folder_path) if f.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".gif"))]
    filenames.extend(image_files)

def crop_images(filenames):
    from PIL import Image
    # image = Image.open("D:\\Downloads\\169\\704429-1-sbj1z.jpg.jpg")
    # resized_image = image.resize((1600, 900))
    # resized_image.save("D:\\Downloads\\169\\704429-1-sbj1z-crop.jpg", format="JPEG", quality=85)
    for f in filenames:
        image = Image.open("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\{}".format(f))
        resized_image = image.resize((1600, 900))
        resized_image.save("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\Transto169\\{}".format(f), format="JPEG", quality=85)
    print("resized image saved!")
    
def Saliency_Detection(filenames):
    import cv2
    for f in filenames:
        img = cv2.imread("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\{}".format(f))
        ...
        cv2.imwrite("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\SaliencyMap\\{}".format(f), cropped)
        # 顯示結果
...
for f, d in zip(filenames, decriptions):
    f = f[:f.find(".jpg")]
    image_path = "C:\\Users\\allen.jiang\\Downloads\\news_imgs\\{}.jpg".format(f)
    print(image_path)
    print(d)
    ...
    append_to_log("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\Yolo\\{}.txt".format(f), f"{d}\n")
    for obj in detected_objects:
        append_to_log("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\Yolo\\{}.txt".format(f), f"{obj['label']} - Confidence: {obj['confidence']:.2f}")
    ...
    if best_match and best_match["similarity"] > 0.1:
        append_to_log("C:\\Users\\allen.jiang\\Downloads\\news_imgs\\Yolo\\{}.txt".format(f), f"Best match: {best_match['label']} with similarity {best_match['similarity']:.2f}")
        output_path = "C:\\Users\\allen.jiang\\Downloads\\news_imgs\\Yolo\\{}_crop.jpg".format(f)
...
'''
name = "701980"
image_path = "D:\\Downloads\\ai_crop\\{}.jpg".format(name)
description = "motorcycle"

device = "cuda" if torch.cuda.is_available() else "cpu"
clip_model, clip_preprocess = clip.load("ViT-B/32", device=device) # 加載 CLIP 模型
yolov5_model = load_yolov5_model() # 加載 YOLOv5 模型
allowed_classes = []
detected_objects = get_detected_objects(yolov5_model, image_path, allowed_classes)
print("Detected objects:")
for obj in detected_objects:
    print(f"{obj['label']} - Confidence: {obj['confidence']:.2f}")
    append_to_log("D:\\Downloads\\ai_crop\\{}.txt".format(name), f"{obj['label']} - Confidence: {obj['confidence']:.2f}")

best_match = match_description_with_objects(image_path, description, detected_objects, clip_model, clip_preprocess, device)
print(best_match)
if best_match and best_match["similarity"] > 0.1:
    print(f"Best match: {best_match['label']} with similarity {best_match['similarity']:.2f}")
    output_path = "D:\\Downloads\\ai_crop\\{}_crop.jpg".format(name)
    cropped_image = crop_and_save_image(image_path, best_match["box"], output_path, target_aspect=16/9)
