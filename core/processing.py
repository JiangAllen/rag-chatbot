...
def pdf_to_img():
    PDF_DIR = Path("./dataset/PATENT_PDF")
    IMG_DIR = Path("./dataset/PATENT_IMG")
    POPPLER_PATH = r"C:\Program Files\poppler\Library\bin"
    IMG_DIR.mkdir(parents=True, exist_ok=True)
    DPI = 300
    for pdf_file in PDF_DIR.glob("*.pdf"):
        pdf_name = pdf_file.stem
        output_folder = IMG_DIR / pdf_name
        ...
        try:
            ...
            for i, image in enumerate(images, start=1):
                output_path = output_folder / f"page_{i:03d}.png"
                image.save(output_path, "PNG")
            print(f"✅ 已完成：{pdf_name}（共 {len(images)} 頁）")
        except Exception as e:
            print(f"❌ 轉換失敗：{pdf_file.name}，原因：{e}")
